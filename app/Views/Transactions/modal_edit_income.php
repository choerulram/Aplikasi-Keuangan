...
<div id="modalEditIncomeTransaction" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-4 md:p-6 relative border-t-4 border-yellow-500 animate-fadeIn">
    <button onclick="toggleEditIncomeTransactionModal(false)" class="absolute top-2 right-2 md:top-3 md:right-3 text-gray-400 hover:text-gray-700 focus:outline-none">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold mb-4 text-yellow-600">Ubah Transaksi Pemasukan</h2>
    </div>
    <form id="formEditIncomeTransaction" method="POST" action="/transactions/income/edit">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="deskripsi" id="edit_income_deskripsi" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-xs md:text-sm font-semibold mb-1">Jumlah</label>
        <input type="number" name="jumlah" id="edit_income_jumlah" min="0" step="0.01" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-xs md:text-sm font-semibold mb-1">Akun</label>
        <select name="account_id" id="edit_income_account_id" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      ...
      <div class="mb-3">
        ...
        <select name="category_id" id="edit_income_category_id" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      ...
      <div class="mb-3">
        ...
        <input type="date" name="tanggal" id="edit_income_tanggal" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <input type="hidden" name="tipe" value="income">
      <div class="flex flex-col md:flex-row md:justify-end mt-4 gap-2">
        <button type="button" onclick="toggleEditIncomeTransactionModal(false)" class="px-4 py-3 w-full md:w-auto rounded bg-gray-200 text-gray-700 hover:bg-gray-300 text-base md:text-sm">Batal</button>
