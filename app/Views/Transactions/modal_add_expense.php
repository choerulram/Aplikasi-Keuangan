...
<div id="modalAddExpenseTransaction" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative border-t-4 border-main animate-fadeIn">
    <button onclick="toggleAddExpenseTransactionModal(false)" class="absolute top-3 right-3 text-gray-400 hover:text-gray-700 focus:outline-none">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-xl font-bold mb-4 text-main">Tambah Transaksi Pengeluaran</h2>
    </div>
    <form id="formAddExpenseTransaction" method="POST" action="/transactions/expense/add">
      <?= csrf_field() ?>
      <div class="mb-3">
        <label class="block text-sm font-semibold mb-1">Deskripsi</label>
        <input type="text" name="deskripsi" placeholder="Contoh: Belanja Bulanan" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-main" required>
      </div>
      <div class="mb-3">
      ...
      <div class="flex justify-end mt-4">
        <button type="button" onclick="toggleAddExpenseTransactionModal(false)" class="px-4 py-2 mr-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300">Batal</button>
