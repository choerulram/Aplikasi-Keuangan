...
<div class="mb-2">
    <h1 class="text-2xl md:text-3xl font-bold text-main tracking-tight drop-shadow-sm mb-4 leading-tight">Transaksi Pemasukan</h1>
</div>
    ...
    <form method="get" action="" class="flex flex-col gap-2 md:flex-row md:flex-wrap md:gap-2 md:items-end flex-1 w-full">
        ...
        <div class="flex gap-2 items-end w-full md:w-auto mt-2 md:mt-0">
            ...
            <?php if (!empty($search) || !empty($account) || !empty($category) || !empty($date)): ?>
                <a href="/transactions/income" class="px-3 py-1.5 md:px-4 md:py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition text-xs md:text-sm">Reset</a>
            <?php endif; ?>
        </div>
    </form>
    <button id="btnShowAddIncomeTransactionModal" type="button" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 bg-main text-white rounded-lg shadow hover:bg-highlight transition h-9 md:h-11 text-sm md:text-base mt-2 md:mt-0">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-2 px-2 md:px-4 text-xs md:text-sm font-bold border-b border-r border-gray-200">
                            <span class="whitespace-nowrap flex items-center justify-start"> 
                                <?php 
                                    $jumlahFormatted = function_exists('format_rupiah') 
                        ...
                        <td class="py-2 px-2 md:px-4 w-40 text-center border-b border-r border-gray-200">
                            <div class="flex flex-col md:flex-row items-center md:justify-center gap-1">
                                ...
                                <button type="button"
                                    ...
                                    title="Ubah"
                                    onclick='toggleEditIncomeTransactionModal(true, <?= json_encode($trx, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP) ?>)'>
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
