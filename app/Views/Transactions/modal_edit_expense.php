...
<div id="modalEditExpenseTransaction" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative border-t-4 border-yellow-500 animate-fadeIn">
    <button onclick="toggleEditExpenseTransactionModal(false)" class="absolute top-3 right-3 text-gray-400 hover:text-gray-700 focus:outline-none">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-xl font-bold mb-4 text-yellow-600">Ubah Transaksi Pengeluaran</h2>
    </div>
    <form id="formEditExpenseTransaction" method="POST" action="/transactions/expense/edit">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="deskripsi" id="edit_expense_deskripsi" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm font-semibold mb-1">Jumlah</label>
        <input type="number" name="jumlah" id="edit_expense_jumlah" min="0" step="0.01" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm font-semibold mb-1">Akun</label>
        <select name="account_id" id="edit_expense_account_id" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-yellow-500" required>
      ...
      <div class="mb-3">
        ...
        <input type="date" name="tanggal" id="edit_expense_tanggal" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <input type="hidden" name="tipe" value="expense">
      <div class="flex justify-end mt-4">
        <button type="button" onclick="toggleEditExpenseTransactionModal(false)" class="px-4 py-2 mr-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300">Batal</button>
