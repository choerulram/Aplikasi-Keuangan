...
<div class="mb-2">
    <h1 class="text-2xl md:text-3xl font-bold text-main tracking-tight drop-shadow-sm mb-4 leading-tight">Kategori Pengeluaran</h1>
</div>
<div class="flex flex-wrap items-end gap-2 mb-6">
    <form method="get" action="" class="flex flex-wrap gap-2 items-end flex-1">
        <div>
            ...
            <input type="text" name="search" id="search" value="<?= esc($search ?? '') ?>" placeholder="Cari nama kategori..." class="px-3 py-2 md:px-3 md:py-2 border rounded-lg focus:outline-none focus:ring w-full md:w-80 text-xs md:text-sm" />
        </div>
        <div class="flex gap-2 items-end">
            ...
            <?php if (!empty($search)): ?>
                <a href="/categories/expense" class="px-3 py-1.5 md:px-4 md:py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition text-xs md:text-sm">Reset</a>
            <?php endif; ?>
        </div>
    </form>
    <a id="btnShowAddCategoryModalExpense" href="#" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 bg-main text-white rounded-lg shadow hover:bg-highlight transition h-9 md:h-11 text-sm md:text-base">
            ...
            <?php if (!empty($categories)): ?>
                ...
                foreach ($categories as $kategori): ?>
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-2 px-2 md:px-4 text-xs md:text-sm text-gray-800 font-semibold border-b border-r border-gray-200"><?= esc($kategori['nama_kategori']) ?></td>
                        <td class="py-2 px-2 md:px-4 text-xs md:text-sm text-gray-600 border-b border-r border-gray-200"><?= esc($kategori['tipe']) ?></td>
                        <td class="py-2 px-2 md:px-4 text-xs md:text-sm text-gray-500 border-b border-r border-gray-200"><?= esc($kategori['created_at']) ?></td>
                        <?php if ($role === 'admin'): ?>
