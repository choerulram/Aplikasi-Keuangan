...
<div id="modalEditCategoryExpense" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-4 md:p-6 relative border-t-4 border-yellow-500 animate-fadeIn">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold mb-4 text-yellow-600">Ubah Kategori Pengeluaran</h2>
    </div>
    <form id="formEditCategoryExpense" method="POST" action="/categories/expense/edit">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="nama_kategori" id="edit_expense_nama_kategori" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <input type="hidden" name="tipe" value="expense">
      <div class="flex flex-col md:flex-row md:justify-end mt-4 gap-2">
        <button type="button" onclick="toggleEditCategoryModalExpense(false)" class="px-4 py-3 w-full md:w-auto rounded bg-gray-200 text-gray-700 hover:bg-gray-300 text-base md:text-sm">Batal</button>
