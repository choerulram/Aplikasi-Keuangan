...
<div class="container mx-auto px-2 md:px-4 py-4 md:py-6">
    <div class="mb-2">
        <h1 class="text-2xl md:text-3xl font-bold text-main tracking-tight drop-shadow-sm mb-4 leading-tight">Perencanaan Pendapatan</h1>
    </div>
        ...
        <form method="get" action="" class="flex flex-wrap gap-2 items-end flex-1">
            ...
            <div class="flex gap-2 items-end">
                ...
                <?php if (!empty($search) || !empty($category) || !empty($periode)): ?>
                    <a href="/budgets/income" class="px-3 py-1.5 md:px-4 md:py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition text-xs md:text-sm">Reset</a>
                <?php endif; ?>
            </div>
        </form>
    ...
    <?= view('Budgets/modal_detail_income_budget', ['isAdmin' => $isAdmin]) ?>
    <?= view('Budgets/modal_delete_income_budget') ?>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-2 px-2 md:px-4 border-b border-r border-gray-200">
                            <div class="flex items-center">
                                ...
                                <span class="ml-2 text-xs md:text-sm <?= $percentage >= 100 ? 'text-green-600' : 'text-blue-600' ?>"><?= number_format($percentage, 1) ?>%</span>
                            </div>
                        </td>
                        <?php if ($isAdmin): ?>
                        ...
                        <td class="py-2 px-2 md:px-4 w-40 text-center border-b border-r border-gray-200">
                            <div class="flex flex-col md:flex-row items-center md:justify-center gap-1">
                                <button type="button"
                                    ...
                                    title="Detail"
                                    onclick='toggleDetailIncomeBudgetModal(true, {
                                        "id": <?= $budget["id"] ?>,
                                ...
                                <button type="button"
                                    ...
                                    title="Hapus"
                                    onclick='toggleDeleteIncomeBudgetModal(true, <?= json_encode($budget, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP) ?>)'>
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
