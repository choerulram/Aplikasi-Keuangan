...
<div id="modalDeleteExpenseBudget" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-4 md:p-7 relative border-t-4 border-red-500 animate-fadeIn">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold text-red-600 mb-2">Hapus Batas Pengeluaran?</h2>
      <p class="text-sm md:text-base text-gray-700 mb-4">Anda yakin ingin menghapus batas pengeluaran untuk kategori <span id="delete_expense_kategori" class="font-semibold text-main"></span>? Tindakan ini tidak dapat dibatalkan.</p>
      <form id="formDeleteExpenseBudget" method="post" action="">
        <input type="hidden" name="_method" value="DELETE">
        <?= csrf_field() ?>
        <div class="flex gap-3 justify-center mt-2 w-full">
          <button type="button" onclick="toggleDeleteExpenseBudgetModal(false)" class="px-4 py-3 flex-1 md:w-auto rounded-lg bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 transition text-base md:text-sm">Batal</button>
