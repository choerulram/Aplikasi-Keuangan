<!-- Modal Hapus User -->
<div id="modalDeleteUser" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-4 md:p-7 relative border-t-4 border-red-500 animate-fadeIn">
    <button onclick="toggleDeleteUserModal(false)" class="absolute top-3 right-3 text-gray-400 hover:text-gray-700 focus:outline-none">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold text-red-600 mb-2">Hapus User?</h2>
      <p class="text-sm md:text-base text-gray-700 mb-4">Anda yakin ingin menghapus user <span id="delete_user_info" class="font-semibold text-main"></span>? Tindakan ini tidak dapat dibatalkan.</p>
      <form id="formDeleteUser" method="post" action="/users/delete">
        <input type="hidden" name="id" id="delete_user_id">
        <?= csrf_field() ?>
        <div class="flex gap-3 justify-center mt-2 w-full">
          <button type="button" onclick="toggleDeleteUserModal(false)" class="px-4 py-3 flex-1 md:w-auto rounded-lg bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 transition text-base md:text-sm">Batal</button>
...
<script>
function toggleDeleteUserModal(show, data = null) {
  const modal = document.getElementById('modalDeleteUser');
  if (show) {
    if (data) {
      document.getElementById('delete_user_id').value = data.id;
      document.getElementById('delete_user_info').textContent = `${data.username} (${data.email})`;
    }
