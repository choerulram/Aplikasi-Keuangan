...
<div id="modalDetailUser" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-4 md:p-6 relative border-t-4 border-blue-500 animate-fadeIn">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold mb-4 text-blue-600">Detail User</h2>
    </div>
    <div class="space-y-5 mt-2">
      <div class="flex flex-col gap-2 bg-white rounded-lg p-3 md:p-4 border border-gray-200">
        <div class="flex flex-col gap-1">
          <span class="text-sm md:text-[13px] text-gray-700 font-semibold uppercase tracking-wide">Username</span>
          <span id="detail_user_username" class="text-base md:text-lg font-bold text-gray-900"></span>
        </div>
        <div class="flex flex-col gap-1 mt-2">
          <span class="text-sm md:text-[13px] text-gray-700 font-semibold uppercase tracking-wide">Email</span>
          <span id="detail_user_email" class="text-base md:text-base text-gray-700"></span>
        </div>
        <div class="flex flex-col gap-1 mt-2">
          <span class="text-sm md:text-[13px] text-gray-700 font-semibold uppercase tracking-wide">Nama</span>
          <span id="detail_user_nama" class="text-base md:text-base text-gray-700"></span>
        </div>
        <div class="flex flex-col gap-1 mt-2">
          <span class="text-sm md:text-[13px] text-gray-700 font-semibold uppercase tracking-wide">Tanggal Dibuat</span>
          <span id="detail_user_created_at" class="text-base md:text-base text-gray-700"></span>
        </div>
        <div class="flex flex-col gap-1 mt-2">
          <span class="text-sm md:text-[13px] text-gray-700 font-semibold uppercase tracking-wide">Role</span>
          <span id="detail_user_role_badge"></span>
        </div>
      </div>
    </div>
    <div class="flex flex-col md:flex-row md:justify-end mt-6 gap-2">
      <button type="button" onclick="toggleDetailUserModal(false)" class="w-full md:w-auto px-4 py-3 rounded-lg bg-blue-500 text-white font-semibold hover:bg-blue-600 transition text-base md:text-sm">Tutup</button>
    </div>
  </div>
</div>
