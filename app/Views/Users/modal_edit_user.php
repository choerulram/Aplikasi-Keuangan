...
<div id="modalEditUser" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden transition-opacity duration-200">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-4 md:p-6 relative border-t-4 border-yellow-500 animate-fadeIn">
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <h2 class="text-lg md:text-xl font-bold mb-4 text-yellow-600">Ubah User</h2>
    </div>
    <form id="formEditUser" method="POST" action="/users/edit">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="username" id="edit_user_username" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm md:text-sm font-semibold mb-1">Email</label>
        <input type="email" name="email" id="edit_user_email" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm md:text-sm font-semibold mb-1">Nama</label>
        <input type="text" name="nama" id="edit_user_nama" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm md:text-sm font-semibold mb-1">Role</label>
        <select name="role" id="edit_user_role" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" required>
          <option value="admin">Admin</option>
          <option value="user">User</option>
        </select>
      </div>
      <div class="mb-3">
        <label class="block text-sm md:text-sm font-semibold mb-1">Password <span class="text-xs text-gray-500">(Kosongkan jika tidak ingin mengubah)</span></label>
        <input type="password" name="password" id="edit_user_password" class="w-full border rounded px-4 py-3 md:px-3 md:py-2 text-base md:text-sm focus:outline-none focus:ring focus:border-yellow-500" autocomplete="new-password" oninput="validateEditUserPassword()">
      ...
      <div class="flex flex-col md:flex-row md:justify-end mt-4 gap-2">
        <button type="button" onclick="toggleEditUserModal(false)" class="px-4 py-3 w-full md:w-auto rounded bg-gray-200 text-gray-700 hover:bg-gray-300 text-base md:text-sm">Batal</button>
        <button id="btnEditUserSubmit" type="submit" class="px-4 py-3 w-full md:w-auto rounded bg-yellow-500 text-white font-semibold hover:bg-yellow-600 transition text-base md:text-sm">Simpan Perubahan</button>
      </div>
    </form>
  </div>
</div>
